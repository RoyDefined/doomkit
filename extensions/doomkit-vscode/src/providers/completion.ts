...
export function registerCompletion(context: vscode.ExtensionContext): void {
    ...
    const provider: vscode.CompletionItemProvider = {
        provideCompletionItems() {
            ...
            const enabled = data.symbols.filter(
                (s) => s.engines.includes(engine) && s.languages.includes(lang),
            );
    ...
    context.subscriptions.push(
        vscode.languages.registerCompletionItemProvider('decorate', provider),
    );
}
